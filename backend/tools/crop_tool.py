...
# Now returns top recommendation + 4 alternatives (matches
# original SmartFarm app behaviour)
# ============================================================
import os
import joblib
import numpy as np
...
def recommend_crop(soil_type: str, weather: dict) -> dict:
    """
    Takes soil type + weather summary and returns:
    - Top crop recommendation with confidence + reasoning + tags
    - Top 4 alternative crops with their confidence scores
    (Mirrors original SmartFarm app's "4 alternative suggestions" feature)
    """
    ...
    crop_name   = encoder.inverse_transform([top_index])[0]
    confidence  = round(float(probabilities[top_index]) * 100, 1)

    alternatives = []
    for idx in sorted_indices[1:5]:
        alt_crop = encoder.inverse_transform([idx])[0]
        alt_conf  = round(float(probabilities[idx]) * 100, 1)
        alternatives.append({
            "crop":       alt_crop,
            "confidence": alt_conf,
            "tags":       CROP_TAGS.get(alt_crop.lower(), [])
        })
